...
每个应用生成到 `outputs/<application-id>/`，其中包含源码、运行说明、业务假设、验证结果、差距清单，以及 `本地预览.html` + `preview-runtime/`（双击即可查看与发布后一致的真实渲染，无需 Node.js；装了 Node 的机器可另按运行说明用 Vite 本地运行）。已有目录默认不会被覆盖；继续迭代时使用同一个 ID，并明确说明要修改的范围。
