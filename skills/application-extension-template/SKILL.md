...
3. Copy `assets/local-preview-template.html` to `outputs/<application-id>/本地预览.html`, then replace the single line `window.__APP_PAYLOAD__ = null;` with `window.__APP_PAYLOAD__ = { appId, manifest, files, entry: "src/App.jsx", confirmItems };` where `files` is byte-identical to step 1 and `confirmItems` lists the open 待确认 items in business language. When serializing the object into the HTML, escape every `</` inside string values as `<\/` (or escape `<` as `\u003c`) — otherwise any `</script` sequence in a source file breaks the HTML parser. This escaping lives in the HTML serialization layer only: the browser parses the strings back to the original bytes, so the in-memory `files` stays byte-identical to the push payload and the 所见即所推 red line is not violated.
4. Agent-verify without Node: the injected JSON parses, `entry` exists in `files`, every relative import inside `files` resolves within `files`, at least one `.css` file is present, no raw `</script` sequence remains inside the injected `<script>` block, and the three `preview-runtime/` files match the skill assets byte-for-byte.
...
2. Call `GET <演示站>/api/preview/policy` with header `x-device-id: <deviceId>` (demo-server address is built into PUSH.md §1 — never ask the user for it). When the local config already stores a gate code, also send `x-gate-code: <code>` so the server can resolve this code's configuration even on a machine that has never pushed; never ask the user for the code just for this fetch. The response's `limits` is the only block to consume: `{ maxFiles, realData, reviewRequired, ttlDays, generationBrief, updatedAt, ... }`.
3. Call `GET <演示站>/api/preview/assets` with the same headers. The response is `{ assets: [{ id, group, desc, url }], expiresInSeconds }` — a machine-specific list of reference-state screenshots for the Basic shell. Download every listed `url` immediately into a session temp directory (the URLs are short-lived signed links); keep the `desc` next to each file. During generation and state verification, use each downloaded image as the visual reference for the state its `desc` names. **The list is whatever the platform granted this machine — use what arrived, generate the rest from the written specs, and never speculate about, request, or mention assets that were not in the list.**
...
- both fetches send only the random `deviceId` header plus, when locally stored, the gate code — never send file contents, requirement text, or any hardware/account identifier with them; the deviceId disclosure duty of PUSH.md §2.1 applies to these fetches too (first-time creation is announced; if the user refuses the identifier, skip both fetches and use the cached/default values with no image references);
...
- a machine with no locally stored gate code is not yet bound to one, so both fetches return the platform default configuration — that is expected, not an error; the code's real configuration applies once the code is stored locally (sent as `x-gate-code`) or from the first successful push onward;
